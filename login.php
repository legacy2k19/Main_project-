...
if(isset($_POST["login"]))
{
  if($validator->validate($_POST))
  {
      if($auth->authenticate($_POST["email"],$_POST["password"]))
      {
		...
		{
        $_SESSION["T"]=$_POST["email"];
		$data=$dao->getData("*","tr_registration","T_Email='".$_POST["email"]."'");
        $_SESSION["T_id"]=$data[0]["T_id"];
        $_SESSION["T_Name"]=$data[0]["T_Name"];
		
			if($_SESSION["Status"]=="B"){ 
			   $msg="you are Blocked by the Admin";
			}
			else{ 
			   $_SESSION["uername"]=$_POST["email"];
		...
		$data = $dao->getData("*","std_registration","S_Email='".$_POST["email"]."'");
        $_SESSION["Reg _id"]=$data[0]["S_id"];
        $_SESSION["S_Name"]=$data[0]["S_Name"];
